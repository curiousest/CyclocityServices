...
function connectDbHandle(callback){
	mongodb.Db.connect(DBURL, function(err, client) {
		if (err) callback(err, null);
		client.collection(DBCOLLECTION, function(err, collection){
			if(err) callback(err, null);
			dbHandle = collection;
			callback(null, null);
		});
...
function writeStationStatus(station, callback){
	async.series([
		function(callback){
			if (!dbHandle) {
				connectDbHandle(function(err, res){callback(err, res);});
			}
			else callback(null, null);
		},
		function(callback){
			dbHandle.insert(station, function(err, res){callback(err, res);});
		}
	],
...
function getStationStatus(city, stationNumber, unixTimeStamp, callback){
	async.series([
		function(callback){
			if (!dbHandle) {
				connectDbHandle(function(err, res){callback(err, res);});
		...
		function(callback){
			var roundedDownTime = (Math.floor(parseInt(unixTimeStamp.toString()) / 300) * 300).toString();
			dbHandle.findOne( {'_id': (city + '-' + stationNumber.toString() + '-' + roundedDownTime)}, 
			function(err, res){
				callback(err, res);
			});
		}
...
exports.writeStationStatus = writeStationStatus;
exports.getStationStatus = getStationStatus;
